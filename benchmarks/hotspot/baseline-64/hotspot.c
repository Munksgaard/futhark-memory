...
int compute_tran_temp(cl_mem MatrixPower, cl_mem MatrixTemp[2], int col,
		      ...
		      int borderRows, double *TempCPU, double *PowerCPU) {
...
void usage(int argc, char **argv) {
  fprintf(stderr,
          "Usage: %s <grid_rows/grid_cols> <pyramid_height> <sim_time> <temp_file> <power_file> <output_file> <runs>\n",
          argv[0]);
  fprintf(stderr,
...
int main(int argc, char **argv) {
  ...
  double* out_buf = malloc(sizeof(double) * size);
  ...
  MatrixTemp[1] =
    clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(double) * size, NULL,
                   &error);
  ...
  for (int i = 0; i<runs+1; i++) {
    ...
    error = clEnqueueWriteBuffer(command_queue, MatrixTemp[0], 1, 0, sizeof(double) * size, FilesavingTemp, 0, 0, 0);
    if (error != CL_SUCCESS)
      fatal_CL(error, __LINE__);

    error = clEnqueueWriteBuffer(command_queue, MatrixPower, 1, 0, sizeof(double) * size, FilesavingPower, 0, 0, 0);
    if (error != CL_SUCCESS)
      fatal_CL(error, __LINE__);
    ...
    error = clEnqueueReadBuffer(command_queue, MatrixTemp[ret], 1, 0, sizeof(double) * size, out_buf, 0, 0, 0);
    if (error != CL_SUCCESS)
      fatal_CL(error, __LINE__);
  ...
  writeoutput(out_buf, grid_rows, grid_cols, ofile);
