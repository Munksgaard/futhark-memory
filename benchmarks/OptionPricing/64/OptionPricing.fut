...
let sobolIndR [m][num_bits] (dir_vs: [m][num_bits]i32) (n: i32): [m]f64 =
  let divisor = 2.0 ** f64.i64(num_bits)
  let arri    = sobolIndI( dir_vs, n )
  in map (\x -> f64.i32(x) / divisor) arri
...
let sobolRecMap [n][num_bits] (sob_fact:  f64, dir_vs: [n][num_bits]i32, (lb_inc, ub_exc): (i32,i32) ): [][]f64 =
  ...
  in  map (\xs -> map (\x -> f64.i32 x * sob_fact) xs) vct_ints
...
let ugaussian [n] (ps: [n]f64): [n]f64 = map ugaussianEl ps
...
let mkPrices [num_und][num_dates]
  ...
  let e_rows = map (\x: [num_und]f64 -> map f64.exp x) c_rows
...
let trajInner(amount: f64, ind: i32, disc: []f64): f64 = amount * #[unsafe] disc[ind]

let payoff1(md_disct: []f64, md_detval: []f64, xss: [1][1]f64): f64 =
...
let payoff2 (md_disc: []f64, xss: [5][3]f64): f64 =
  let (date, amount) =
...
let payoff3(md_disct: []f64, xss: [367][3]f64): f64 =
  let conds  = map (\x ->
...
let genericPayoff(contract: i32) (md_disct: []f64) (md_detval: []f64) (xss: [][]f64): f64 =
  if      contract == 1 then #[unsafe] payoff1(md_disct, md_detval, xss :> [1][1]f64)
  else if contract == 2 then #[unsafe] payoff2(md_disct, xss :> [5][3]f64)
  else if contract == 3 then #[unsafe] payoff3(md_disct, xss :> [367][3]f64)
  else 0.0
...
let main [k][num_bits][num_models][num_und][num_dates][num_discts]
        ...
        (md_cs: [num_models][num_und][num_und]f64)
        (md_vols: [num_models][num_dates][num_und]f64)
        (md_drifts: [num_models][num_dates][num_und]f64)
        (md_sts: [num_models][num_und]f64)
        (md_detvals: [num_models][1]f64)
        (md_discts: [num_models][num_discts]f64)
        (bb_inds: [3][num_dates]i32)
        (bb_data: [3][num_dates]f64)
         : []f64 =
  ...
  let payoffs   = map (\bb_row: [num_models]f64  ->
  ...
  in  map (/f64.i32 num_mc_it) payoff
