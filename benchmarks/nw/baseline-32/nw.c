...
void usage(int argc, char **argv)
{
  ...
  fprintf(stderr, "\t<runs> - number of benchmark runs");
  exit(1);
}
...
int main(int argc, char **argv){
  ...
  long int max_rows, max_cols, penalty;
  int runs;
  stopwatch sw;
  ...
  if (argc == 5)
    {
      max_rows = (long int)atoi(argv[1]);
      max_cols = (long int)atoi(argv[1]);
      penalty = (long int)atoi(argv[2]);
      tempchar = argv[3];
      runs = (long int)atoi(argv[4]);
    }
  else{
    usage(argc, argv);
  }
  ...
  //initialization
  for (long int i = 0 ; i < max_cols; i++){
    for (long int j = 0 ; j < max_rows; j++){
  ...
  if(nworkitems < 1 || workgroupsize < 0){
    fprintf(stderr, "ERROR: invalid or missing <num_work_items>[/<work_group_size>]\n");
    return -1;
  }
  ...
  if(err != CL_SUCCESS) { fprintf(stderr, "ERROR: clCreateBuffer input_item_set (size:%d) => %d\n", max_cols * max_rows, err); return -1;}
  reference_d		 = clCreateBuffer(context, CL_MEM_READ_WRITE, max_cols * max_rows * sizeof(int), NULL, &err );
  if(err != CL_SUCCESS) { fprintf(stderr, "ERROR: clCreateBuffer reference (size:%d) => %d\n", max_cols * max_rows, err); return -1;}
  output_itemsets_d = clCreateBuffer(context, CL_MEM_READ_WRITE, max_cols * max_rows * sizeof(int), NULL, &err );
  if(err != CL_SUCCESS) { fprintf(stderr, "ERROR: clCreateBuffer output_item_set (size:%d) => %d\n", max_cols * max_rows, err); return -1;}

  int worksize = max_cols - 1;
  fprintf(stderr, "worksize = %d\n", worksize);
  ...
  for (int run = 0; run < runs + 1; run++) {
    ...
    if(err != CL_SUCCESS) { fprintf(stderr, "ERROR: clEnqueueWriteBuffer bufIn1 (size:%d) => %d\n", max_cols * max_rows, err); return -1; }
    err = clEnqueueWriteBuffer(cmd_queue, reference_d, 1, 0, max_cols * max_rows * sizeof(int), reference, 0, 0, 0);
    if(err != CL_SUCCESS) { fprintf(stderr, "ERROR: clEnqueueWriteBuffer bufIn2 (size:%d) => %d\n", max_cols * max_rows, err); return -1; }
