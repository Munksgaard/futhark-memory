...
def lud_diagonal [b] (a: [b][b]f64): *[b][b]f64 =
  map (\mat ->
         let mat = copy mat
         in #[unsafe]
            loop (mat: *[b][b]f64) for i < b-1 do
    ...
    let a2s =
        map (\a1 ->
               map (\row0 -> -- Upper
                      ...
                      let sum = loop sum=0.0f64 for k < i do
                                  sum + diag[i,k] * row[k]
                      let row[i] = row[i] - sum
                      in  row
                   ) a1
...
def lud_perimeter_lower [b][m] (diag: [b][b]f64) (mat: [m][b][b]f64): *[m][b][b]f64 =
  map (\blk ->
         map (\row0 -> -- Lower
                #[unsafe]
...
def lud_internal [m][b] (top_per: [m][b][b]f64) (lft_per: [m][b][b]f64) (mat_slice: [m][m][b][b]f64): *[m][m][b][b]f64 =
