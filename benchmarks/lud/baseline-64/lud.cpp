...
void lud_cuda(double *d_m, int matrix_dim);
...
int main(int argc, char *argv[]) {
	...
	clCreateBuffer(context, CL_MEM_READ_WRITE,
		       matrix_dim * matrix_dim * sizeof(double), NULL, &err);
    if (err != CL_SUCCESS) {
	...
	err =
	    clEnqueueWriteBuffer(cmd_queue, d_m, 1, 0,
				 matrix_dim * matrix_dim * sizeof(double), mm, 0,
				 0, 0);
	if (err != CL_SUCCESS) {
	...
	for (i = 0; i < matrix_dim - BLOCK_SIZE; i += BLOCK_SIZE) {
	    ...
	    clSetKernelArg(internal, 1,
			   sizeof(double) * BLOCK_SIZE * BLOCK_SIZE,
			   (void *)NULL);
	    clSetKernelArg(internal, 2, sizeof(double) * BLOCK_SIZE * BLOCK_SIZE,
			   (void *)NULL);
	...
	clSetKernelArg(diagnal, 0, sizeof(void *), (void *)&d_m);
	clSetKernelArg(diagnal, 1, sizeof(double) * BLOCK_SIZE * BLOCK_SIZE,
		       (void *)NULL);
	clSetKernelArg(diagnal, 2, sizeof(cl_int), (void *)&matrix_dim);
	clSetKernelArg(diagnal, 3, sizeof(cl_int), (void *)&i);
	...
	err =
	    clEnqueueReadBuffer(cmd_queue, d_m, 1, 0,
				matrix_dim * matrix_dim * sizeof(double), m, 0,
				0, 0);
	if (err != CL_SUCCESS) {
